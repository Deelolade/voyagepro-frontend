...
const Bookings = () => {
  ...
  const changeBookingStatus = async (token) => {
    const res = await axios.patch(`${API_URL}/admin/dashboard/admin/68bb5e9d97e79c1f3d5d1af5/status`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const results = useQueries({
    queries: [
      ...
      {
        queryKey: ['status'],
        queryFn: () => changeBookingStatus(token)
      }
    ]
  })

  const [bookingQuery, bookingStatus] = results;
  console.log(bookingQuery.data)
  // const { data: bookings, isLoading, isError } = useQueries({
          ...
          <div className="h-96 overflow-y-auto scrollbar-hide py-2">
            {
              bookingQuery.data?.length > 0 ?
                bookingQuery.data.map((booking, idx) => {
                  return (
